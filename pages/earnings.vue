  ...
  <div class="space-y-4 animate-fade-in max-w-7xl mx-auto pb-10">
    ...
    <div class="flex items-center justify-between gap-4 mb-4 mt-2">
      ...
      <div class="flex-1 relative max-w-2xl">
        <h1 class="text-2xl font-bold text-gray-900 tracking-tight font-display mb-1">Earnings & Payouts</h1>
        <p class="text-gray-500 text-sm font-medium">Track your delivery tips, base fees, and withdrawal history.</p>
      </div>
      <button v-if="balance > 0" @click="showWithdrawDrawer = true" class="px-6 py-2.5 bg-parentPrimary text-white rounded-full font-bold text-xs shadow-sm shadow-parentPrimary/20 hover:brightness-110 active:scale-[0.98] transition-all uppercase tracking-widest flex items-center gap-2">
        Request Payout
      </button>
    </div>
    ...
    <div v-else class="space-y-8 animate-fade-in">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white p-8 rounded-[2.5rem] relative overflow-hidden group border border-gray-100 shadow-sm">
          ...
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 relative z-10">Available for Payout</p>
          ...
          <div class="flex items-center gap-2 relative z-10">
            ...
            <p class="text-[10px] text-emerald-600 font-bold uppercase tracking-wider">Verified Balance</p>
          </div>
        </div>
        ...
        <div class="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-200/50 shadow-inner relative overflow-hidden">
          ...
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Life-time Earnings</p>
      ...
      <section class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-xl font-bold text-gray-900 tracking-tight">Settlement Settings</h3>
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div class="space-y-4">
            <label class="text-[10px] font-bold text-gray-400 tracking-widest uppercase ml-1">Payout Schedule</label>
            <div class="flex gap-2 p-1.5 bg-gray-100 rounded-2xl">
              <button v-for="p in ['instant', 'weekly']" :key="p"
                @click="handleUpdateFrequency(p)"
                class="flex-1 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all"
          ...
          <div class="space-y-4">
            <label class="text-[10px] font-bold text-gray-400 tracking-widest uppercase ml-1">Verified Receiving Account</label>
            <div class="p-5 rounded-2xl bg-gray-50 border border-gray-200/50 space-y-2 relative overflow-hidden">
              ...
              <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ wallet?.bankDetails?.accountName || 'Not configured' }}</p>
            </div>
            <button @click="showBankDrawer = true" class="text-[11px] font-bold text-parentPrimary uppercase tracking-widest hover:underline flex items-center gap-1 group">
      ...
      <section class="mt-8 space-y-4">
        <div class="flex items-center justify-between px-2">
          <h3 class="font-bold text-gray-900 tracking-tight text-xl">Earnings History</h3>
          <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ transactions.length }} Events</span>
        </div>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-gray-100">
                  <th class="py-4 px-6 font-medium text-gray-400 text-[11px] whitespace-nowrap">Errand Details</th>
                  <th class="py-4 px-6 font-medium text-gray-400 text-[11px] whitespace-nowrap text-right">Credit / Debit</th>
                  <th class="py-4 px-6 font-medium text-gray-400 text-[11px] whitespace-nowrap text-right">Timestamp</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50/50">
                <tr v-for="tx in transactions" :key="tx._id" class="hover:bg-gray-50/80 transition-colors group cursor-default">
                  <td class="py-4 px-6 min-w-[200px]">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p v-if="tx.order" class="text-[10px] text-gray-400 font-medium tracking-wider mt-0.5 uppercase">Order #{{ tx.order.slice(-8) }}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="py-4 px-6 text-right">
                    <p class="text-[11px] text-gray-900 font-medium uppercase">{{ new Date(tx.createdAt).toLocaleDateString() }}</p>
    ...
    <SideDrawer :isOpen="showBankDrawer" @close="showBankDrawer = false">
        <div class="space-y-6">
          <div class="border-b border-gray-100 pb-4">
            <h3 class="text-2xl font-bold text-gray-900 tracking-tight font-display">Bank Information</h3>
            <p class="text-gray-500 text-sm mt-1 font-medium text-balance">Your earnings will be settled to this verified account.</p>
          </div>

          <div class="space-y-5">
            <div class="space-y-1.5">
              <label class="text-[10px] font-bold text-gray-400 uppercase ml-1 tracking-widest">Select Financial Institution</label>
            ...
            <div class="space-y-1.5">
              <label class="text-[10px] font-bold text-gray-400 uppercase ml-1 tracking-widest">Account Number</label>
              <div class="relative">
                ...
                <button 
                  ...
                  class="absolute right-2 top-1.5 bottom-1.5 px-4 rounded-lg bg-gray-900 text-white text-[10px] font-bold uppercase tracking-widest disabled:opacity-30 hover:bg-black transition-all shadow-lg shadow-gray-900/10"
                >
            ...
            <Transition name="fade">
              <div v-if="bankForm.accountName" class="p-4 rounded-xl bg-emerald-50 border border-emerald-100 space-y-1 shadow-inner">
                <p class="text-[9px] text-emerald-600 font-bold uppercase tracking-widest">Verified Beneficiary</p>
                <p class="text-sm text-emerald-900 font-bold uppercase">{{ bankForm.accountName }}</p>
              </div>
            </Transition>
          </div>
          
          <div class="pt-6 border-t border-gray-100 flex gap-4 mt-6">
            <button @click="handleSaveBank" :disabled="!isAccountVerified" class="w-full py-4 bg-parentPrimary text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-xl shadow-parentPrimary/20 disabled:opacity-30 hover:brightness-110 active:scale-[0.98] transition-all">Update Bank Account</button>
          </div>
        </div>
    </SideDrawer>
    ...
    <SideDrawer :isOpen="showWithdrawDrawer" @close="showWithdrawDrawer = false">
        <div class="flex flex-col items-center justify-center py-6 border-b border-gray-100 space-y-4">
          ...
          <div class="text-center">
            <h3 class="text-2xl font-bold text-gray-900 tracking-tight mb-1 font-display">Request Payout</h3>
            <p class="text-gray-500 text-sm font-medium text-balance">Fast-tracked withdrawal to your linked settlement account.</p>
          </div>
        </div>
        ...
        <div class="space-y-6 pt-6">
          <div class="space-y-2">
            ...
            <p class="text-[10px] text-gray-400 uppercase font-black tracking-widest text-center">Available: ₦{{ balance?.toLocaleString() }}</p>
          </div>

          <div class="pt-6 border-t border-gray-100 flex flex-col gap-3">
            <button @click="handleWithdraw" :disabled="withdrawAmount <= 0 || withdrawAmount > (balance || 0)" class="w-full py-4 bg-parentPrimary text-white rounded-xl font-bold text-xs shadow-xl shadow-parentPrimary/20 hover:brightness-110 disabled:opacity-30 active:scale-[0.98] transition-all uppercase tracking-widest">
              Confirm Withdrawal
            </button>
          </div>
        </div>
